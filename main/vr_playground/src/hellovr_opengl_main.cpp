...
// 		m_SecondsTimer.start(1000, this);
	...
	vr::VRInput()->SetActionManifestPath( "data/hellovr_actions.json");
...
{
    const std::string strFullPath = "data/textures/cube_texture.png";
	
	
	int nImageWidth, nImageHeight , channelNmb;
	auto* imageRGBA = stbi_load(strFullPath.c_str(), &nImageWidth, &nImageHeight, &channelNmb, 0);
	...
	glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, nImageWidth, nImageHeight,
		0, GL_RGB, GL_UNSIGNED_BYTE, &imageRGBA[0] );
...
void CMainApplication::SetupScene()
{
	...
	matTransform = glm::translate(matTransform,
		glm::vec3(-( (float)m_iSceneVolumeWidth * m_fScaleSpacing ) / 2.f,
	...
	for( int z = 0; z< m_iSceneVolumeDepth; z++ )
	{
		for( int y = 0; y< m_iSceneVolumeHeight; y++ )
		{
			for( int x = 0; x< m_iSceneVolumeWidth; x++ )
			{
				...
				mat = glm::translate(mat, glm::vec3(m_fScaleSpacing, 0, 0 ));
			}
			mat = glm::translate(mat, glm::vec3(-((float)m_iSceneVolumeWidth) * m_fScaleSpacing, m_fScaleSpacing, 0 ));
		}
		mat = glm::translate(mat, glm::vec3( 0, -((float)m_iSceneVolumeHeight) * m_fScaleSpacing, m_fScaleSpacing) );
	}
...
void CMainApplication::RenderScene( vr::Hmd_Eye nEye )
{
	...
	if( m_bShowCubes )
	{
		...
		const auto sceneMatrix = GetCurrentViewProjectionMatrix(nEye);
		glUniformMatrix4fv( m_nSceneMatrixLocation, 1, GL_FALSE, &sceneMatrix[0][0] );
		glBindVertexArray( m_unSceneVAO );
		glBindTexture( GL_TEXTURE_2D, m_iTexture );
	...
	if( bIsInputAvailable )
	{
		...
		const auto controllerMatrix = GetCurrentViewProjectionMatrix(nEye);
		glUniformMatrix4fv( m_nControllerMatrixLocation, 1, GL_FALSE, &controllerMatrix[0][0]);
		glBindVertexArray( m_unControllerVAO );
