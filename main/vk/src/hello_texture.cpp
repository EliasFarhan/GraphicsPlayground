...
namespace vk
{
VkVertexInputBindingDescription HelloTexture::Vertex::GetBindingDescription()
{
...
std::array<VkVertexInputAttributeDescription, 3> HelloTexture::Vertex::GetAttributeDescriptions()
{
...
void HelloTexture::CreateGraphicsPipeline()
{
    ...
    core::BufferFile vertexShaderFile = filesystem.LoadFile(
        "data/shaders/06_hello_texture/texture.vert.spv");
    core::BufferFile fragmentShaderFile = filesystem.LoadFile(
        "data/shaders/06_hello_texture/texture.frag.spv");
...
void HelloTexture::CreateCommands()
{
...
void HelloTexture::CreateDescriptorPool()
{
    ...
    poolSizes[1].descriptorCount = static_cast<uint32_t>(swapchain.images.size()); VkDescriptorPoolCreateInfo poolInfo{};
...
void HelloTexture::UpdateUniformBuffers(core::seconds duration)
{
    auto& engine = Engine::GetInstance();
    auto& renderer = engine.GetRenderer();
    auto& swapchain = engine.GetSwapchain();
...
void HelloTexture::CreateTextureImageView()
{
    ...
    if (vkCreateImageView(driver.device, &viewInfo, nullptr, &textureImageView_) != VK_SUCCESS) {
...
void HelloTexture::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout,
